...
export default function AdminPage({ params }: { params: { slug: string } }) {
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    ...
    if (dishesInCategory.length > 0) {
      alert(`Esta categoría tiene ${dishesInCategory.length} platos. Mueve o elimina los platos primero.`)
      return
    }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-5xl mx-auto px-4 md:px-6 py-4 flex justify-between items-center">
          <div>
            <h1 className="text-lg md:text-xl font-bold text-gray-900">Panel Admin</h1>
                    ...
                    return (
                      <li key={cat.id} className="flex items-center justify-between px-5 py-4 hover:bg-gray-50">
                        <div>
                          ...
                          <p className="text-xs text-gray-400 mt-0.5">{count} platos</p>
                        </div>
                        <button
              ...
              <div id="form-top" className="bg-white rounded-2xl shadow-sm p-5 mb-6 border border-orange-100">
                ...
                <div className="flex justify-between items-center mb-5">
                  ...
                  <div className="flex items-center gap-1.5 bg-orange-50 px-3 py-1.5 rounded-full">
                    ...
                    <span className="text-xs text-orange-600 font-medium">Auto EN/DE/FR</span>
                  </div>
                </div>
                ...
                <div className="space-y-5">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {[
                        ...
                        >
                          ...
                          />
                          <div>
                            <p className="text-sm font-medium text-gray-800 leading-tight">{opt.label}</p>
                  ...
                  <div>
                    ...
                    {form.allergen_ids.length > 0 && (
                      <p className="text-xs text-red-500 mt-2 font-medium">
                        {form.allergen_ids.length} alérgeno{form.allergen_ids.length > 1 ? 's' : ''} seleccionado{form.allergen_ids.length > 1 ? 's' : ''}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex gap-3 pt-2 border-t border-gray-100">
                    ...
                    >
                      {translating
                        ? <><span className="animate-spin inline-block">🌐</span> Traduciendo...</>
                        : formSaving
                ...
                <div key={dish.id} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-0.5">{getCategoryName(dish.category_id)}</p>
                      <p className="text-base font-black text-orange-500 mt-1">{Number(dish.price).toFixed(2)} €</p>
                      <div className="flex gap-1 mt-1 flex-wrap">
                        {dish.is_featured && <span className="text-xs">⭐</span>}
                        {dish.is_vegan && <span className="text-xs">🌱</span>}
                        {dish.is_meat && <span className="text-xs">🥩</span>}
                        {dish.is_gluten_free && <span className="text-xs">🌾</span>}
                        {dish.dish_allergens?.map(da => (
                          <span key={da.allergens.id} className="text-xs">{da.allergens.icon}</span>
                        ))}
                      </div>
                    </div>
                  </div>
            ...
            <div className="hidden md:block bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
              <table className="w-full">
                <thead className="bg-gray-50 border-b border-gray-100">
                  <tr>
                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Plato</th>
                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Categoría</th>
                    <th className="text-left px-6 py-4 text-xs font-semibold text-gray-500 uppercase">Precio</th>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="flex gap-1 mt-0.5 flex-wrap">
                              ...
                              {dish.is_gluten_free && <span className="text-xs">🌾</span>}
                              {dish.dish_allergens?.map(da => (
                                <span key={da.allergens.id} className="text-xs" title={da.allergens.name}>
                                  {da.allergens.icon}
                                </span>
                              ))}
                            </div>
