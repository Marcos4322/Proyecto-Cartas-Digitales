...
const UI: Record<string, Record<Lang, string>> = {
  ...
  noResults: {
    es: "No hay platos con estos filtros",
    en: "No dishes match your filters",
    de: "Keine Gerichte",
    fr: "Aucun plat",
  },
  clearFilters: {
  ...
  powered: {
    es: "Carta digital por",
    en: "Digital menu by",
    de: "Digitale Karte von",
    fr: "Menu par",
  },
  tabForYou: {
    es: "✨ Para ti",
  ...
  noForYou: {
    es: "No encontramos platos que coincidan con tus preferencias. Prueba a cambiarlas.",
    en: "No dishes match your preferences. Try changing them.",
    de: "Keine passenden Gerichte.",
    fr: "Aucun plat correspondant.",
  },
  goToMenu: {
    es: "Ver carta completa",
    en: "See full menu",
    de: "Zur Speisekarte",
    fr: "Voir la carte",
  },
  like: { es: "Me gusta", en: "Like", de: "Gefallt mir", fr: "Jaime" },
};
...
function dishDesc(dish: Dish, lang: Lang): string {
  ...
  if (lang === "de")
    return dish.description_de || dish.description_en || dish.description;
  if (lang === "fr")
    return dish.description_fr || dish.description_en || dish.description;
...
function filterByPreferences(dishes: Dish[], prefs: WizardPreferences): Dish[] {
  return dishes.filter((dish) => {
    ...
    if (prefs.diet === "vegan" && (!dish.is_vegan || dish.is_meat))
      return false;
    if (prefs.diet === "vegetarian" && (!dish.is_vegetarian || dish.is_meat))
      return false;
...
function DishCard({
  ...
  // Cargar likes por restaurante
  useEffect(() => {
    const likedDishes = JSON.parse(
      localStorage.getItem(`menuai_likes_${restaurantId}`) || "[]",
    );
    if (likedDishes.includes(dish.id)) setLiked(true);
  }, [dish.id, restaurantId]);

  // Tracking vista del plato
  useEffect(() => {
  ...
  const handleLike = async () => {
    ...
    // Guardar likes por restaurante
    const likedDishes = JSON.parse(
      localStorage.getItem(`menuai_likes_${restaurantId}`) || "[]",
    );
    if (liked) {
      localStorage.setItem(
        `menuai_likes_${restaurantId}`,
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {likesCount > 0 ? (
                ...
                <span className="text-gray-300">{t("like", lang)}</span>
              )}
            </button>
          </div>
        </div>
...
export default function MenuClient({ data }: Props) {
  ...
  // Clave unica por restaurante
  const PREFS_KEY = `menuai_preferences_${restaurant.id}`;
  ...
  const forYouDishes = preferences
    ? filterByPreferences(dishes, preferences)
    : [];

  const filteredDishes = dishes.filter((dish) => {
    if (activeCategory !== "all" && dish.category_id !== activeCategory)
      return false;
    if (activeDiet === "vegan" && (!dish.is_vegan || dish.is_meat))
      return false;
    if (activeDiet === "vegetarian" && (!dish.is_vegetarian || dish.is_meat))
      return false;
        ...
        {activeTab === "menu" && (
          <div className="border-t border-gray-50">
            ...
            <div className="max-w-2xl mx-auto px-4 pb-3">
              <div className="flex gap-2 overflow-x-auto pb-1">
                {DIET_FILTERS.map((filter) => (
                  <button
                    ...
                    onClick={() =>
                      setActiveDiet(
                        activeDiet === filter.key ? null : filter.key,
                      )
                    }
                ...
                {/* Boton alergenos — solo si hay alergenos */}
              ...
              {/* Panel alergenos — solo si hay alergenos */}
